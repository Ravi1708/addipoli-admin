...
import { updatehub, listhubDetails, listpartners } from "../actions/hubActions";
import { FormContainer } from "../components/FormContainer";
import { GoogleMap, LoadScript, Marker } from "@react-google-maps/api";
import Geocode from "react-geocode";
import Select from "react-select";

const EditHub = ({ history, match }) => {
  const hubid = match.params.id;
  ...
  const hubUpdate = useSelector((state) => state.hubUpdate);
  const {
  ...
  useEffect(() => {
    ...
    if (successUpdate) {
      ...
      if (!hub.hubName || hub.hubId !== hubid) {
        dispatch(listhubDetails(hubid));
      } else {
        sethubId(hub.hubId);
        sethubName(hub.hubName);
        setpassword(hub.password);
        setinchargeName(hub.inchargeName);
        setinchargeNumber(hub.inchargeNumber);
        setcity(hub.city);
        setstates(hub.states);
        setpincode(hub.pincode);
        setcountry(hub.country);
        setaddress(hub.address);
        // setpartnerIds(hub.partnerIds);
        setlatitude(hub.latitude);
        setlongitude(hub.longitude);
      }
    }
  }, [dispatch, history, hubid, hub, successUpdate]);
  ...
  const handleitems = (e) => {
    setpartnerIds(Array.isArray(e) ? e.map((x) => x.value) : [hub.partnerIds]);
  };
